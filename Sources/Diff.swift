...
func diff(lhs: String, _ rhs: String) -> (Range<Int>, String)? {
	let result = diff(Array(lhs.characters), Array(rhs.characters))
	return result.flatMap { ($0.0, String($0.1)) }
}

func diff<T: Equatable>(lhs: [T], _ rhs: [T]) -> (Range<Int>, [T])? {
	return diff(lhs, rhs, compare: ==)
}

func diff<T>(lhs: [T], _ rhs: [T], compare: (T, T) -> Bool) -> (Range<Int>, [T])? {
