...
class CursorTests: XCTestCase {
	...
	func testCursorToRangeMulti() {
		...
		XCTAssertEqual(NSRange(location: 7, length: 5), cursor.range(with: string))
	}
	...
	func testCursorToRangeSingle() {
		...
		XCTAssertEqual(NSRange(location: 7, length: 1), cursor.range(with: string))
	}
	...
	func testCursorToRangeZero() {
		...
		XCTAssertEqual(NSRange(location: 0, length: 0), cursor.range(with: string))
	}
}
