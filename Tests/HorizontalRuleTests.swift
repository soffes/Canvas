...
class HorizontalRuleTests: XCTestCase {
	func testParsing() {
		...
		var block = HorizontalRule(string: native, range: range, enclosingRange: range)!
		...
		block = HorizontalRule(string: native, range: range, enclosingRange: range)!
		XCTAssertEqual(NSRange(location: 0, length: range.length - 1), block.nativePrefixRange)
		XCTAssertEqual(NSRange(location: range.length - 1, length: 1), block.visibleRange)
		...
		native = "+++"
		range = NSRange(location: 0, length: (native as NSString).length)
		XCTAssertNil(HorizontalRule(string: native, range: range, enclosingRange: range))
